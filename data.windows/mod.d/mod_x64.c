...
#define _CMT CMT
extern void  mod_x64_ode_solver_solvedata (rx_solve *solve){
  _solveData = solve;
}
extern rx_solve *mod_x64_ode_solver_get_solvedata(){
  return _solveData;
}
SEXP mod_x64_model_vars();
double _theta[14];
extern double* mod_x64_theta(double *theta){
  _theta[0] = theta[0]; _theta[1] = theta[1]; _theta[2] = theta[2]; _theta[3] = 66500.0000000000000000; _theta[4] = 55000.0000000000000000; _theta[5] = 362.4599999999999795; _theta[6] = theta[3]; _theta[7] = theta[4]; _theta[8] = theta[5]; _theta[9] = theta[6]; _theta[10] = theta[7]; _theta[11] = theta[8]; _theta[12] = theta[9]; _theta[13] = theta[10];
  return _theta;
}


// prj-specific differential eqns
void mod_x64_dydt(int *_neq, double t, double *__zzStateVar__, double *__DDtStateVar__)
{
  int _cSub = _neq[1];
  double   Age,
  P01_Age,
  ...
  Age=P01_Age+t/safe_zero(365.0)/safe_zero(24.0);
  Bw=P01_Bw;
  C01_Cu=C01_A/safe_zero(C01_V);
  F=C01_Cu/safe_zero(MW_COR)*1E6;
  TA=C01_ALB/safe_zero(MW_ALB)*1E6;
  TC=C01_CBG/safe_zero(MW_CBG)*1E6;
  TCI=(1-C01_RC)*TC;
  TCE=C01_RC*TC;
  KA=C01_Kca/safe_zero(MW_ALB)*1E6;
  KCI=C01_Kci/safe_zero(MW_CBG)*1E6;
  KCE=C01_Kce/safe_zero(MW_CBG)*1E6;
...
void mod_x64_calc_jac(int *_neq, double t, double *__zzStateVar__, double *__PDStateVar__, unsigned int __NROWPD__) {
  int _cSub=_neq[1];
  (&_solveData->subjects[_cSub])->jac_counter[0]++;
}
// Functional based initial conditions.
void mod_x64_inis(int _cSub, double *__zzStateVar__){
}
// prj-specific derived vars
void mod_x64_calc_lhs(int _cSub, double t, double *__zzStateVar__, double *_lhs) {
...
double mod_x64_F(int _cSub,  int _cmt, double _amt, double t){
 return _amt;
}
// Functional based absorption lag
double mod_x64_Lag(int _cSub,  int _cmt, double t){
 return t;
  double   Age,
  P01_Age,
  Bw,
...
double mod_x64_Rate(int _cSub,  int _cmt, double _amt, double t){
 return 0.0;
  double   Age,
  P01_Age,
  Bw,
...
double mod_x64_Dur(int _cSub,  int _cmt, double _amt, double t){
 return 0.0;
}
// Model Times
void mod_x64_mtime(int _cSub, double *_mtime){
}
extern SEXP mod_x64_model_vars(){
  int pro=0;
  SEXP _mv = PROTECT(_rxGetModelLib("mod_x64_model_vars"));pro++;
  ...
  SET_STRING_ELT(lhs,17,mkChar("fu_Var"));
    ...
    REAL(ini)[2] = 362.4599999999999800;
    ...
    SET_STRING_ELT(mmd5,0,mkChar("0a581346586637b07bda5c44e3dc16c7"));
    SET_STRING_ELT(mmd5n,1,mkChar("parsed_md5"));
    SET_STRING_ELT(mmd5,1,mkChar("66ea2f9bf50c0c2592f73c300007a9d7"));
    SET_STRING_ELT(trann,0,mkChar("lib.name"));
    SET_STRING_ELT(tran, 0,mkChar("mod_x64"));
    SET_STRING_ELT(trann,1,mkChar("jac"));
    SET_STRING_ELT(tran,1,mkChar("fullint"));
    SET_STRING_ELT(trann,2,mkChar("prefix"));
    SET_STRING_ELT(tran, 2,mkChar("mod_x64_"));
    SET_STRING_ELT(trann,3,mkChar("dydt"));
    SET_STRING_ELT(tran, 3,mkChar("mod_x64_dydt"));
    SET_STRING_ELT(trann,4,mkChar("calc_jac"));
    SET_STRING_ELT(tran, 4,mkChar("mod_x64_calc_jac"));
    SET_STRING_ELT(trann,5,mkChar("calc_lhs"));
    SET_STRING_ELT(tran, 5,mkChar("mod_x64_calc_lhs"));
    SET_STRING_ELT(trann,6,mkChar("model_vars"));
    SET_STRING_ELT(tran, 6,mkChar("mod_x64_model_vars"));
    SET_STRING_ELT(trann,7,mkChar("theta"));
    SET_STRING_ELT(tran, 7,mkChar("mod_x64_theta"));
    SET_STRING_ELT(trann,8,mkChar("inis"));
    SET_STRING_ELT(tran, 8,mkChar("mod_x64_inis"));
    SET_STRING_ELT(trann,  9,mkChar("dydt_lsoda"));
    SET_STRING_ELT(tran,   9,mkChar("mod_x64_dydt_lsoda"));
    SET_STRING_ELT(trann,10,mkChar("calc_jac_lsoda"));
    SET_STRING_ELT(tran, 10,mkChar("mod_x64_calc_jac_lsoda"));
    SET_STRING_ELT(trann,11,mkChar("ode_solver_solvedata"));
    SET_STRING_ELT(tran, 11,mkChar("mod_x64_ode_solver_solvedata"));
    SET_STRING_ELT(trann,12,mkChar("ode_solver_get_solvedata"));
    SET_STRING_ELT(tran, 12,mkChar("mod_x64_ode_solver_get_solvedata"));
    SET_STRING_ELT(trann,13,mkChar("dydt_liblsoda"));
    SET_STRING_ELT(tran, 13,mkChar("mod_x64_dydt_liblsoda"));
    SET_STRING_ELT(trann,14,mkChar("F"));
    SET_STRING_ELT(tran, 14,mkChar("mod_x64_F"));
    SET_STRING_ELT(trann,15,mkChar("Lag"));
    SET_STRING_ELT(tran, 15,mkChar("mod_x64_Lag"));
    SET_STRING_ELT(trann,16,mkChar("Rate"));
    SET_STRING_ELT(tran, 16,mkChar("mod_x64_Rate"));
    SET_STRING_ELT(trann,17,mkChar("Dur"));
    SET_STRING_ELT(tran, 17,mkChar("mod_x64_Dur"));
    SET_STRING_ELT(trann,18,mkChar("mtime"));
    SET_STRING_ELT(tran, 18,mkChar("mod_x64_mtime"));
    SET_STRING_ELT(trann,19,mkChar("assignFuns"));
    SET_STRING_ELT(tran, 19,mkChar("mod_x64_assignFuns"));
...
extern void mod_x64_dydt_lsoda(int *neq, double *t, double *A, double *DADT)
{
  mod_x64_dydt(neq, *t, A, DADT);
}
extern int mod_x64_dydt_liblsoda(double t, double *y, double *ydot, void *data)
{
  int *neq = (int*)(data);
  mod_x64_dydt(neq, t, y, ydot);
  return(0);
}
extern void mod_x64_calc_jac_lsoda(int *neq, double *t, double *A,int *ml, int *mu, double *JAC, int *nrowpd){
  // Update all covariate parameters
  mod_x64_calc_jac(neq, *t, A, JAC, *nrowpd);
}

//Create function to call from R's main thread that assigns the required functions. Sometimes they don't get assigned.
extern void mod_x64_assignFuns(){
  _assignFuns();
}

//Initialize the dll to match RxODE's calls
void R_init0_mod_x64(){
  // Get C callables on load; Otherwise it isn't thread safe
  _assignFuns();
  R_RegisterCCallable("mod_x64","mod_x64_assignFuns", (DL_FUNC) mod_x64_assignFuns);
  R_RegisterCCallable("mod_x64","mod_x64_theta", (DL_FUNC) mod_x64_theta);
  R_RegisterCCallable("mod_x64","mod_x64_inis",(DL_FUNC) mod_x64_inis);
  R_RegisterCCallable("mod_x64","mod_x64_dydt",(DL_FUNC) mod_x64_dydt);
  R_RegisterCCallable("mod_x64","mod_x64_calc_lhs",(DL_FUNC) mod_x64_calc_lhs);
  R_RegisterCCallable("mod_x64","mod_x64_calc_jac",(DL_FUNC) mod_x64_calc_jac);
  R_RegisterCCallable("mod_x64","mod_x64_dydt_lsoda", (DL_FUNC) mod_x64_dydt_lsoda);
  R_RegisterCCallable("mod_x64","mod_x64_calc_jac_lsoda", (DL_FUNC) mod_x64_calc_jac_lsoda);
  R_RegisterCCallable("mod_x64","mod_x64_ode_solver_solvedata", (DL_FUNC) mod_x64_ode_solver_solvedata);
  R_RegisterCCallable("mod_x64","mod_x64_ode_solver_get_solvedata", (DL_FUNC) mod_x64_ode_solver_get_solvedata);
  R_RegisterCCallable("mod_x64","mod_x64_F", (DL_FUNC) mod_x64_F);
  R_RegisterCCallable("mod_x64","mod_x64_Lag", (DL_FUNC) mod_x64_Lag);
  R_RegisterCCallable("mod_x64","mod_x64_Rate", (DL_FUNC) mod_x64_Rate);
  R_RegisterCCallable("mod_x64","mod_x64_Dur", (DL_FUNC) mod_x64_Dur);
  R_RegisterCCallable("mod_x64","mod_x64_mtime", (DL_FUNC) mod_x64_mtime);
  R_RegisterCCallable("mod_x64","mod_x64_dydt_liblsoda", (DL_FUNC) mod_x64_dydt_liblsoda);
}
//Initialize the dll to match RxODE's calls
void R_init_mod_x64(DllInfo *info){
  // Get C callables on load; Otherwise it isn't thread safe
  R_init0_mod_x64();
  static const R_CallMethodDef callMethods[]  = {
    {"mod_x64_model_vars", (DL_FUNC) &mod_x64_model_vars, 0},
    {NULL, NULL, 0}
  };
...
void R_unload_mod_x64 (DllInfo *info){
  // Free resources required for single subject solve.
  SEXP _mv = PROTECT(_rxGetModelLib("mod_x64_model_vars"));
  if (!isNull(_mv)){
    _rxRmModelLib("mod_x64_model_vars");
  }
  UNPROTECT(1);
}
