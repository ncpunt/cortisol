...
#define _CMT CMT
extern void  mod__ode_solver_solvedata (rx_solve *solve){
  _solveData = solve;
}
extern rx_solve *mod__ode_solver_get_solvedata(){
  return _solveData;
}
SEXP mod__model_vars();
double _mod___t[14];
extern double* mod__theta(double *theta){
  _mod___t[0] = theta[0]; _mod___t[1] = theta[1]; _mod___t[2] = theta[2]; _mod___t[3] = 66500.0000000000000000; _mod___t[4] = 55000.0000000000000000; _mod___t[5] = 362.4599999999999795; _mod___t[6] = theta[3]; _mod___t[7] = theta[4]; _mod___t[8] = theta[5]; _mod___t[9] = theta[6]; _mod___t[10] = theta[7]; _mod___t[11] = theta[8]; _mod___t[12] = theta[9]; _mod___t[13] = theta[10];
  return _mod___t;
}


// prj-specific differential eqns
void mod__dydt(int *_neq, double t, double *__zzStateVar__, double *__DDtStateVar__)
{
  int _cSub = _neq[1];
  double   Age,
  P01_Age,
...
void mod__calc_jac(int *_neq, double t, double *__zzStateVar__, double *__PDStateVar__, unsigned int __NROWPD__) {
  int _cSub=_neq[1];
  (&_solveData->subjects[_cSub])->jac_counter[0]++;
}
// Functional based initial conditions.
void mod__inis(int _cSub, double *__zzStateVar__){
}
// prj-specific derived vars
void mod__calc_lhs(int _cSub, double t, double *__zzStateVar__, double *_lhs) {
...
double mod__F(int _cSub,  int _cmt, double _amt, double t){
 return _amt;
}
// Functional based absorption lag
double mod__Lag(int _cSub,  int _cmt, double t){
 return t;
  double   Age,
  P01_Age,
  Bw,
...
double mod__Rate(int _cSub,  int _cmt, double _amt, double t){
 return 0.0;
  double   Age,
  P01_Age,
  Bw,
...
double mod__Dur(int _cSub,  int _cmt, double _amt, double t){
 return 0.0;
}
// Model Times
void mod__mtime(int _cSub, double *_mtime){
}
extern SEXP mod__model_vars(){
  int pro=0;
  SEXP _mv = PROTECT(_rxGetModelLib("mod__model_vars"));pro++;
  ...
  SET_STRING_ELT(lhs,17,mkChar("fu_Var"));
    ...
    SET_STRING_ELT(mmd5,0,mkChar("5a802737257605e1d895656b12475a3f"));
    SET_STRING_ELT(mmd5n,1,mkChar("parsed_md5"));
    SET_STRING_ELT(mmd5,1,mkChar("45d029639585e8cedc0c2f787bf72956"));
    SET_STRING_ELT(trann,0,mkChar("lib.name"));
    SET_STRING_ELT(tran, 0,mkChar("mod_"));
    SET_STRING_ELT(trann,1,mkChar("jac"));
    SET_STRING_ELT(tran,1,mkChar("fullint"));
    SET_STRING_ELT(trann,2,mkChar("prefix"));
    SET_STRING_ELT(tran, 2,mkChar("mod__"));
    SET_STRING_ELT(trann,3,mkChar("dydt"));
    SET_STRING_ELT(tran, 3,mkChar("mod__dydt"));
    SET_STRING_ELT(trann,4,mkChar("calc_jac"));
    SET_STRING_ELT(tran, 4,mkChar("mod__calc_jac"));
    SET_STRING_ELT(trann,5,mkChar("calc_lhs"));
    SET_STRING_ELT(tran, 5,mkChar("mod__calc_lhs"));
    SET_STRING_ELT(trann,6,mkChar("model_vars"));
    SET_STRING_ELT(tran, 6,mkChar("mod__model_vars"));
    SET_STRING_ELT(trann,7,mkChar("theta"));
    SET_STRING_ELT(tran, 7,mkChar("mod__theta"));
    SET_STRING_ELT(trann,8,mkChar("inis"));
    SET_STRING_ELT(tran, 8,mkChar("mod__inis"));
    SET_STRING_ELT(trann,  9,mkChar("dydt_lsoda"));
    SET_STRING_ELT(tran,   9,mkChar("mod__dydt_lsoda"));
    SET_STRING_ELT(trann,10,mkChar("calc_jac_lsoda"));
    SET_STRING_ELT(tran, 10,mkChar("mod__calc_jac_lsoda"));
    SET_STRING_ELT(trann,11,mkChar("ode_solver_solvedata"));
    SET_STRING_ELT(tran, 11,mkChar("mod__ode_solver_solvedata"));
    SET_STRING_ELT(trann,12,mkChar("ode_solver_get_solvedata"));
    SET_STRING_ELT(tran, 12,mkChar("mod__ode_solver_get_solvedata"));
    SET_STRING_ELT(trann,13,mkChar("dydt_liblsoda"));
    SET_STRING_ELT(tran, 13,mkChar("mod__dydt_liblsoda"));
    SET_STRING_ELT(trann,14,mkChar("F"));
    SET_STRING_ELT(tran, 14,mkChar("mod__F"));
    SET_STRING_ELT(trann,15,mkChar("Lag"));
    SET_STRING_ELT(tran, 15,mkChar("mod__Lag"));
    SET_STRING_ELT(trann,16,mkChar("Rate"));
    SET_STRING_ELT(tran, 16,mkChar("mod__Rate"));
    SET_STRING_ELT(trann,17,mkChar("Dur"));
    SET_STRING_ELT(tran, 17,mkChar("mod__Dur"));
    SET_STRING_ELT(trann,18,mkChar("mtime"));
    SET_STRING_ELT(tran, 18,mkChar("mod__mtime"));
    SET_STRING_ELT(trann,19,mkChar("assignFuns"));
    SET_STRING_ELT(tran, 19,mkChar("mod__assignFuns"));
...
extern void mod__dydt_lsoda(int *neq, double *t, double *A, double *DADT)
{
  mod__dydt(neq, *t, A, DADT);
}
extern int mod__dydt_liblsoda(double t, double *y, double *ydot, void *data)
{
  int *neq = (int*)(data);
  mod__dydt(neq, t, y, ydot);
  return(0);
}
extern void mod__calc_jac_lsoda(int *neq, double *t, double *A,int *ml, int *mu, double *JAC, int *nrowpd){
  // Update all covariate parameters
  mod__calc_jac(neq, *t, A, JAC, *nrowpd);
}

//Create function to call from R's main thread that assigns the required functions. Sometimes they don't get assigned.
extern void mod__assignFuns(){
  _assignFuns();
}
...
void R_init0_mod_(){
  ...
  _assignFuns();
  R_RegisterCCallable("mod_","mod__assignFuns", (DL_FUNC) mod__assignFuns);
  R_RegisterCCallable("mod_","mod__theta", (DL_FUNC) mod__theta);
  R_RegisterCCallable("mod_","mod__inis",(DL_FUNC) mod__inis);
  R_RegisterCCallable("mod_","mod__dydt",(DL_FUNC) mod__dydt);
  R_RegisterCCallable("mod_","mod__calc_lhs",(DL_FUNC) mod__calc_lhs);
  R_RegisterCCallable("mod_","mod__calc_jac",(DL_FUNC) mod__calc_jac);
  R_RegisterCCallable("mod_","mod__dydt_lsoda", (DL_FUNC) mod__dydt_lsoda);
  R_RegisterCCallable("mod_","mod__calc_jac_lsoda", (DL_FUNC) mod__calc_jac_lsoda);
  R_RegisterCCallable("mod_","mod__ode_solver_solvedata", (DL_FUNC) mod__ode_solver_solvedata);
  R_RegisterCCallable("mod_","mod__ode_solver_get_solvedata", (DL_FUNC) mod__ode_solver_get_solvedata);
  R_RegisterCCallable("mod_","mod__F", (DL_FUNC) mod__F);
  R_RegisterCCallable("mod_","mod__Lag", (DL_FUNC) mod__Lag);
  R_RegisterCCallable("mod_","mod__Rate", (DL_FUNC) mod__Rate);
  R_RegisterCCallable("mod_","mod__Dur", (DL_FUNC) mod__Dur);
  R_RegisterCCallable("mod_","mod__mtime", (DL_FUNC) mod__mtime);
  R_RegisterCCallable("mod_","mod__dydt_liblsoda", (DL_FUNC) mod__dydt_liblsoda);
}
//Initialize the dll to match RxODE's calls
void R_init_mod_(DllInfo *info){
  // Get C callables on load; Otherwise it isn't thread safe
  R_init0_mod_();
  static const R_CallMethodDef callMethods[]  = {
    {"mod__model_vars", (DL_FUNC) &mod__model_vars, 0},
    {NULL, NULL, 0}
  };
...
void R_unload_mod_ (DllInfo *info){
  // Free resources required for single subject solve.
  SEXP _mv = PROTECT(_rxGetModelLib("mod__model_vars"));
  if (!isNull(_mv)){
    _rxRmModelLib("mod__model_vars");
  }
  UNPROTECT(1);
}
